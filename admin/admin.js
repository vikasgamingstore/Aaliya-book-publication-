...
async function handleProfileSave(e) {
  ...
  const fields = ["company_name","logo_url","favicon_url","description","facebook_url","instagram_url","twitter_url","youtube_url"];
...
async function openRegDetail(id) {
  ...
  box.innerHTML = `
    ...
    ${r.payment_screenshot_url ? `<p><a href="#" onclick="openSecureFile(event, '${r.payment_screenshot_url}')">View Payment Screenshot</a> &nbsp; UTR: ${r.registration_utr || "—"}</p>` : `<p class="field-hint">Customer ne abhi screenshot upload nahi kiya.</p>`}
    <div class="form-grid">
      <div class="field">
...
async function loadPaymentQueue() {
  ...
  tbody.innerHTML = regs.map(r => `
    <tr>
      <td>${r.profiles?.full_name || "—"}</td>
      <td>${r.projects?.project_name || "—"}</td>
      <td>₹${r.projects?.registration_fee || 0}</td>
