...
async function handleSignup(e) {
  e.preventDefault();
  const restoreBtn = typeof lockSubmitButton === "function" ? lockSubmitButton(e.target, "Account ban raha hai...") : () => {};
  ...
  const bankName = v("su-bank-name"), bankAccountName = v("su-bank-account-name");
  const bankAccount = v("su-bank-account"), bankIfsc = v("su-bank-ifsc");
  const idType = document.getElementById("su-id-type").value, idNumber = v("su-id-number");
  ...
  const error = runValidations([
    ...
    [Validate.ifsc(bankIfsc), "IFSC code sahi format mein nahi hai (jaise SBIN0001234)."],
    [Validate.accountNumber(bankAccount), "Bank account number sahi nahi lag raha."],
  ]);
  ...
  if (await mobileAlreadyRegistered(mobile)) {
    ...
    msg.classList.add("error");
    btn.disabled = false; btn.textContent = "Account Banaiye";
    return;
  }
  ...
  // Resolve referral code to a referrer, if one was supplied
  let referrerId = null;
  if (referralCode) {
    try {
